...
class AuthController {
    ...
    async register({ request, response }) {
        ...
        const { email, username, nik, password } = request.only([
            'email',
            'nik',
            'username',
            'password'
        ...
        const validation = await validate({ email, username, nik, password }, rules);

        if (!validation.fails()) {
            try {
                const user = await User.create({ email, username, nik, password });
